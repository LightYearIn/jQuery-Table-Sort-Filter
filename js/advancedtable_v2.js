(function($){
	$.fn.advancedtable = function(custom) {
		...
		//LBK
		// Define searchOn Drop Down if needed
		if(settings.searchOnField != ""){	
			$(settings.searchOnField).show();
			var defaultOption = $('<option>');
			defaultOption.attr('value','-1').text('Everything');
			$(settings.searchOnField).append(defaultOption);

			//START Populate Search On Drop Down values
			// START TR LOOP
			table.find('thead tr').each(function () { 
				...
				$(this).find('th').each(function () {
					...
					//Read the Column Headers and populate them as OPTIONS in the dropdown. 
					var strcell = stripHtml(this.innerHTML);
					var newOption = $('<option>');
					newOption.attr('value',i.toString()).text(strcell);
					$(settings.searchOnField).append(newOption);
			...
			// END TR LOOP
			//END Populate Search On Drop Down values
		}
		  	...
		  	table.find('tbody tr').each(function () { 
				...
				$(this).find('td').each(function () { 		  
					...
					if((settings.searchColumn==0) || (settings.searchColumn==i)){
						...
						//LBK
						//START Search update to include search on column
						/*
						...
						//Search based on the Column selected in Search On Field
						if(settings.searchOnField != "" && $(settings.searchOnField).val().toString() != "-1"){
							if(parseInt($(settings.searchOnField).val()) == i-1){
								// If string is found in this cell
								if((strcell.indexOf(strsearch) > -1)){
									found = true;  
								}
						...
						else{ //if no column is selected or Everything is selected, universal search
							// If string is found in this cell
							if((strcell.indexOf(strsearch) > -1)){
								found = true;  
							}
						}
						//END Search update to include search on column
					}
		...
		//To check if a value is present in an array.
		...
		//To retrieve unique values in an array
